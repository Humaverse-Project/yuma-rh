...
function PlaningFormationScreen() {
    ...
    const data = {formations: formation, salaries: salarie, dateunique: formation.reduce((dates, formation) => {
    ...
    const filtrerDonnees = () => {
        ...
        setdatesUniques(formationsFiltrees.reduce((dates, formation) => {
            if (!dates.includes(formation.date_heure_debut)) {
            dates.push(formation.date_heure_debut);
            }
            return dates;
                    ...
                    >
                        <Card sx={{ display: 'flex', width: "auto", m: 2,
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column'}}>
                                    <CardContent sx={{ flex: '1 0 auto', color: 'black.main' }}>
                                    <Grid container spacing={2}>
                                            ...
                                            <FormGroup>
                                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                                    <DatePicker 
                                                        label="De la date"
                                                        name="dateDebut"
                                                        onChange={handledateDebutFiltresChange}
                                                        size="small"
                                                        format="DD-MM-YYYY"
                                                ...
                                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                                    <DatePicker 
                                                        label="A la date"
                                                        name="dateFin"
                                                        onChange={handledatefinFiltresChange}
                                                        size="small"
                                                        format="DD-MM-YYYY"
                                                        slotProps={{ textField: { size: 'small' } }}
                                                    />
                                        ...
                                        >
                                            <Box
                                                sx={{ display: 'flex', width: "auto", m: 2,
                                                [theme.breakpoints.up('lg')]: {
                                                    pt: 6,
                                                },
                                                [theme.breakpoints.down('sm')]: {
                                            ...
                                            >
                                                <Button variant="contained" onClick={filtrerDonnees} color="blue">Recherche</Button>
                                            </Box>
                                        </Grid>
                                    </Grid>
                                    </CardContent>
                        ...
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow className="table-header-row">
                                    <TableCell className="table-cell-first-prime cell-header" p={10} width="150px">Salarié</TableCell>
                                    
                                    {datesUniques.map((date, index) => (
                                        <TableCell className="table-cell cell-header" key={index}>{new Date(date).toLocaleDateString()}</TableCell>
                                    ))}
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {salariesFiltres.map((salarie) => (
                                    <TableRow key={salarie.id} >
                                        <TableCell className="table-cell-first table-cell"  p={10}>{salarie.nom_salarie}</TableCell>
                                    {datesUniques.map((date, index) => {
                                        ...
                                        return (
                                            <TableCell className="table-cell" key={index} style={cellStyle}>{formation ? formation.nom_formation : ''}</TableCell>
                                        );
                                    })}
                                    </TableRow>
                                ))}
